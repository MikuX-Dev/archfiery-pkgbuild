...
yay -S --noconfirm --needed eww-x11
cp -r /home/builder/.cache/yay/eww/*.pkg.tar.* /home/builder/output-small

pkgsize=$(du -s *.pkg.tar.* | cut -f1)
if [ "$pkgsize" -gt $((100 * 1024)) ]; then
  echo "$package >= 100MB"
  cp -r /home/builder/.cache/pikaur/pkg/*.pkg.tar.* /home/builder/output-large
else
  cp -r /home/builder/.cache/pikaur/pkg/*.pkg.tar.* /home/builder/output-small
fi
